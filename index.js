...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN ASU!?
            ...
            + 'DEV\n' // NAMA OWNER BOT
            + 'FN:Creator Robby Pratama W;\n' // KASIH CREDITS GUA FADHIL GRAPHY SU!!!
            + 'TEL;type=CELL;type=VOICE;waid=6285277262021:+62 852-7726-2021\n' // GANTI NOMOR LU
            + 'END:VCARD'
prefix = '/'
prex = ''
blocked = []
...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const cond = body.slice(0).trim().split(/ +/).shift().toLowerCase()
			...
			const ownerNumber = ["6285277262021@s.whatsapp.net"] // GANTI NOMOR LU
			const adminbotnumber = ["6285277262021@s.whatsapp.net"] // GANTI NOMOR LU
			const frendsowner = ["6285277262021@s.whatsapp.net"] // GAUSAH DIGANTI!!!
			...
			switch(budy){ 
				...
				case `assalamu'alaikum`:
				reply ('Walaikumsalam Bro , Ada Yang Bisa Di Bantu ?')
				break
				}
			...
			switch(command) {
				...
				case 'sticker':
					...
					break
			case 'owner':
                 client.sendMessage(from, {displayname: "ᴏʙʏĐᷢeͦꪜᷨ", vcard: vcard}, MessageType.contact, { quoted: mek})
				...
				case 'testime':
					...
					client.sendMessage(from, '20', text) // ur cods
					client.sendMessage(from, '30', text)
					client.sendMessage(from, '40', text) // ur cods
					client.sendMessage(from, '50', text) // ur cods
					client.sendMessage(from, '60', text) 
					client.sendMessage(from, '70', text) // ur cods
					client.sendMessage(from, '80', text) // ur cods
					client.sendMessage(from, '90', text) 
					client.sendMessage(from, '100', text) // ur cods
					client.sendMessage(from, 'TimeDone !', text) // ur cods
				...
				case 'dare':
					...
					break				
                case `assalamualaikum`:
               client.sendMessage(from, `Waalaikumsalam Mas :)`)
                break
                case 'speed':
                ...
                case 'report':
                    ...
                    client.sendMessage('6285277262021@s.whatsapp.net', options, text, {quoted: mek})
				...
				case 'randomcat':
					reply(mess.wait)
					anu = await fetchJson(`https://api.vhtear.com/randomcat?apikey=NOT-PREMIUM`)
					if (anu.error) return reply(anu.error)
               ...
               case 'infogempa':
			    ...
			    case 'tagall':
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘ᴏʙʏĐᷢeͦꪜᷨ〙', members_id, true)
					break
				case 'clearall':
					if (!isOwner) return reply('Kamu siapa?')
